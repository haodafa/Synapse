...
async function makeSandbox(): Promise<Sandbox> {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "synapse-skill-sync-"));
...
describe("syncSkills", () => {
  ...
  it("overwrites on-disk skill content when the bundle differs", async () => {
    await writeBundleSkill(sandbox.sourceDir, "synapse", {
      "SKILL.md": "new synapse content",
    });
    const onDiskSkill = path.join(sandbox.agentsDir, "synapse");
    await fs.mkdir(onDiskSkill, { recursive: true });
    await fs.writeFile(path.join(onDiskSkill, "SKILL.md"), "old synapse content");
    ...
    const agentsContent = await fs.readFile(
      path.join(sandbox.agentsDir, "synapse", "SKILL.md"),
      "utf-8",
    );
    expect(agentsContent).toBe("new synapse content");
    const claudeContent = await fs.readFile(
      path.join(sandbox.claudeDir, "synapse", "SKILL.md"),
      "utf-8",
    );
    expect(claudeContent).toBe("new synapse content");
    const codexContent = await fs.readFile(
      path.join(sandbox.codexDir, "synapse", "SKILL.md"),
      "utf-8",
    );
    expect(codexContent).toBe("new synapse content");
  });

  it("installs new bundled skills, including references/, when not present on disk", async () => {
    await writeBundleSkill(sandbox.sourceDir, "synapse-epic", {
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["synapse-epic"],
    });

    expect(await fs.readFile(path.join(sandbox.agentsDir, "synapse-epic", "SKILL.md"), "utf-8")).toBe(
      "epic content",
    );
    expect(
      await fs.readFile(
        path.join(sandbox.agentsDir, "synapse-epic", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");
    expect(
      await fs.readFile(
        path.join(sandbox.codexDir, "synapse-epic", "references", "roles.md"),
        "utf-8",
      ),
    ).toBe("roles content");

    const claudeSkillDir = path.join(sandbox.claudeDir, "synapse-epic");
  ...
  it("leaves on-disk skills not in the bundle untouched", async () => {
    await writeBundleSkill(sandbox.sourceDir, "synapse", { "SKILL.md": "new" });
  ...
  it("skips skills listed in skillNames that are missing from the bundle without raising", async () => {
    await writeBundleSkill(sandbox.sourceDir, "synapse", { "SKILL.md": "content" });
    // "synapse-removed" is in skillNames but not in the bundle on disk.
    ...
    const result = await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["synapse", "synapse-removed"],
      onSkillError: (name, error) => errors.push({ name, error }),
    });

    expect(errors).toEqual([]);
    expect(result.processedSkills).toBe(1);
    expect(await fs.readFile(path.join(sandbox.agentsDir, "synapse", "SKILL.md"), "utf-8")).toBe(
      "content",
    );
    await expect(fs.access(path.join(sandbox.agentsDir, "synapse-removed"))).rejects.toThrow();
  });

  it("leaves on-disk skill content alone when the skill has been removed from the bundle", async () => {
    await writeBundleSkill(sandbox.sourceDir, "synapse", { "SKILL.md": "current" });
    const deprecatedDir = path.join(sandbox.agentsDir, "synapse-deprecated");
    ...
    await syncSkills({
      ...
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
      skillNames: ["synapse", "synapse-deprecated"],
    });
  ...
  it("reports per-skill errors via onSkillError without throwing", async () => {
    ...
    await fs.writeFile(path.join(sandbox.agentsDir, "synapse"), "blocking file");
    ...
    expect(errors).toContain("synapse");
...
describe("removeSkill", () => {
  ...
  it("removes the skill from all three targets when present", async () => {
    ...
    await removeSkill("synapse", {
      agentsDir: sandbox.agentsDir,
      claudeDir: sandbox.claudeDir,
      codexDir: sandbox.codexDir,
    });

    await expect(fs.access(path.join(sandbox.agentsDir, "synapse"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.claudeDir, "synapse"))).rejects.toThrow();
    await expect(fs.access(path.join(sandbox.codexDir, "synapse"))).rejects.toThrow();
  });
