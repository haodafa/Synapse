...
export function registerNotificationHandlers(): void {
  ipcMain.handle("synapse:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("synapse:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("synapse:event:notification-click", payload);
      }
