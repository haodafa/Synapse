...
import { createTestSynapseDaemon } from "../src/server/test-utils/synapse-daemon.js";
...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome = process.env.SYNAPSE_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".paseo");
  let frozenHomeRoot = process.env.SYNAPSE_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.SYNAPSE_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  ...
  if (process.env.SYNAPSE_BENCHMARK_REUSE_FROZEN_HOME === "1") {
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestSynapseDaemon({ paseoHomeRoot: frozenHomeRoot, cleanup: false });
