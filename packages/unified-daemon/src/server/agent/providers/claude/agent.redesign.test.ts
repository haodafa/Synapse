...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "SYNAPSE_ENV_SENTINEL_SECRET";
  const runtimeSecret = "SYNAPSE_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "SYNAPSE_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.SYNAPSE_TEST_SENTINEL_SECRET;
  process.env.SYNAPSE_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        SYNAPSE_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.SYNAPSE_TEST_SENTINEL_SECRET;
    } else {
      process.env.SYNAPSE_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
