...
describe("paseo env contract", () => {
  ...
  const baseEnv = {
    ...
    PATH: "/usr/bin",
    SYNAPSE_AGENT_ID: "agent-123",
    SYNAPSE_DESKTOP_MANAGED: "1",
    [SYNAPSE_NODE_ENV]: "production",
    SYNAPSE_SUPERVISED: "1",
  };
  const runtimeControlEnvKeys = [
    "ELECTRON_RUN_AS_NODE",
    "SYNAPSE_NODE_ENV",
    "SYNAPSE_DESKTOP_MANAGED",
    "SYNAPSE_SUPERVISED",
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.SYNAPSE_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.SYNAPSE_DESKTOP_MANAGED).toBeUndefined();
    expect(env[SYNAPSE_NODE_ENV]).toBeUndefined();
    expect(env.SYNAPSE_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.SYNAPSE_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[SYNAPSE_NODE_ENV]).toBeUndefined();
    expect(command.env.SYNAPSE_SUPERVISED).toBeUndefined();
  });
  ...
  test("does not use user NODE_ENV as Paseo runtime mode", () => {
    expect(resolveSynapse NodeEnv({ NODE_ENV: "development" })).toBeUndefined();
    expect(resolveSynapse NodeEnv({ NODE_ENV: "development", SYNAPSE_NODE_ENV: "production" })).toBe(
      "production",
    );
    expect(resolveSynapse NodeEnv({ NODE_ENV: "test", SYNAPSE_NODE_ENV: "local" })).toBeUndefined();
  });
});
