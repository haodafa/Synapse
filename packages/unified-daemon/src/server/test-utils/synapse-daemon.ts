...
interface TestSynapseDaemonOptions {
  downloadTokenTtlMs?: number;
  corsAllowedOrigins?: string[];
  listen?: string;
  logger?: Parameters<typeof createSynapseDaemon>[1];
  mcpDebug?: boolean;
  ...
  voiceLlmProvider?: SynapseDaemonConfig["voiceLlmProvider"];
  voiceLlmProviderExplicit?: boolean;
  voiceLlmModel?: string | null;
  dictationFinalTimeoutMs?: number;
  auth?: SynapseDaemonConfig["auth"];
  pushNotificationSender?: PushNotificationSender;
}

export interface TestSynapseDaemon {
  config: SynapseDaemonConfig;
  daemon: Awaited<ReturnType<typeof createSynapseDaemon>>;
  port: number;
  paseoHome: string;
  staticDir: string;
  close: () => Promise<void>;
...
export async function createTestSynapseDaemon(
  options: TestSynapseDaemonOptions = {},
): Promise<TestSynapseDaemon> {
  const maxAttempts = 8;
