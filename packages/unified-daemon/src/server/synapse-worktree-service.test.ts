...
import {
  ...
  type CreateSynapseWorktreeDeps,
} from "./synapse-worktree-service.js";
import { readSynapseWorktreeMetadata } from "../utils/worktree-metadata.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createSynapseWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createSynapseWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createSynapseWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  const created = await createSynapseWorktree(
    {
      cwd: repoDir,
  ...
  expect(readSynapseWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "synapse-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
interface TestDeps extends CreateSynapseWorktreeDeps {
