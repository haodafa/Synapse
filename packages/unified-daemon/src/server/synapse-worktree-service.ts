...
import {
  markSynapseWorktreeFirstAgentBranchAutoNameAttempted,
...
import type { FirstAgentContext } from "@synapse/protocol/messages";

export interface CreateSynapseWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
}

export interface CreateSynapseWorktreeResult {
...
export type CreateSynapseWorktreeFn = (
...
export interface CreateSynapseWorktreeDeps extends CreateWorktreeCoreDeps {
  projectRegistry: Pick<ProjectRegistry, "get" | "upsert">;
  workspaceRegistry: Pick<WorkspaceRegistry, "get" | "list" | "upsert">;
  workspaceGitService: WorkspaceGitService;
}

export async function createSynapseWorktree(
  input: CreateSynapseWorktreeInput,
  deps: CreateSynapseWorktreeDeps,
): Promise<CreateSynapseWorktreeResult> {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readSynapseWorktreeMetadata>;
  try {
    metadata = readSynapseWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markSynapseWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeSynapseWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
...
async function upsertWorkspaceForWorktree(options: {
  ...
  repoRoot: string;
  worktree: WorktreeConfig;
  deps: Pick<CreateSynapseWorktreeDeps, "projectRegistry" | "workspaceRegistry">;
