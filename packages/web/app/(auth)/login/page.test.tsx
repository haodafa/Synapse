...
import enCommon from "@synapse/views/locales/en/common.json";
import enAuth from "@synapse/views/locales/en/auth.json";
import enSettings from "@synapse/views/locales/en/settings.json";
...
vi.mock("@synapse/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@synapse/core/auth")>(
      "@synapse/core/auth",
    );
...
describe("LoginPage", () => {
  ...
  it("renders login form with email input and continue button", () => {
    ...
    expect(screen.getByText("Sign in to Synapse")).toBeInTheDocument();
  ...
  it("calls sendCode with email on submit", async () => {
    ...
    await user.type(screen.getByLabelText("Email"), "test@synapse.ai");
    await user.click(screen.getByRole("button", { name: "Continue" }));

    await waitFor(() => {
      expect(mockSendCode).toHaveBeenCalledWith("test@synapse.ai");
    });
  });
  ...
  // the synapse:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@synapse.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "synapse://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open Synapse Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
