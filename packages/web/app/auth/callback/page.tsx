...
import { paths, resolvePostAuthDestination } from "@synapse/core/paths";
...
import {
  ...
  CardDescription,
  CardContent,
} from "@synapse/ui/components/ui/card";
    ...
    if (isDesktop) {
      ...
      api
        ...
        .then(({ token }) => {
          ...
          window.location.href = `synapse://auth/callback?token=${encodeURIComponent(token)}`;
        })
        .catch((err) => {
      ...
      loginWithGoogle(code, redirectUri)
        .then(async (loggedInUser) => {
          ...
          //    login on app.synapse.ai instead of clicking the email link,
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Opening Synapse</CardTitle>
            <CardDescription>
              You should see a prompt to open the Synapse desktop app. If
          ...
          <CardContent className="flex justify-center">
            <Button
              variant="outline"
              onClick={() => {
                window.location.href = `synapse://auth/callback?token=${encodeURIComponent(desktopToken)}`;
              }}
            >
              Open Synapse Desktop
            </Button>
          </CardContent>
        </Card>
      </div>
