 ...
 *   synapse-desktop-{version}-mac-{arch}.{dmg|zip}
 *   synapse-desktop-{version}-windows-{arch}.exe
 *   synapse-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^synapse-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
