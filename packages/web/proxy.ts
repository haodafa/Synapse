...
import { LOCALE_COOKIE } from "@synapse/core/i18n";
import {
  SYNAPSE_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-synapse-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(SYNAPSE_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("synapse_logged_in");
