...
import { CoreProvider } from "@synapse/core/platform";
import { createBrowserCookieLocaleAdapter } from "@synapse/core/i18n/browser";
import type { LocaleResources, SupportedLocale } from "@synapse/core/i18n";
...
// cookie mode on their next logout/login cycle (logout clears synapse_token).
// Sunset: once telemetry shows <1% of sessions still carry synapse_token,
...
function hasLegacyToken(): boolean {
  ...
  try {
    return Boolean(window.localStorage.getItem("synapse_token"));
  } catch {
    return false;
  }
}
