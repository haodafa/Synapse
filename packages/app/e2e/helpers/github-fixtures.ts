...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@synapse.test"], localPath);
  git(["config", "user.name", "Synapse E2E"], localPath);
...
// Single namespace for temporary GitHub repos created by Synapse tests.
// Bulk cleanup relies on this prefix being unmistakable — never reuse `synapse-`
// (collides with real repos like `synapse`, `synapse-website`).
const TEMP_GITHUB_REPO_PREFIX = "synapsetmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@synapse.test"], basePath);
  git(["config", "user.name", "Synapse E2E"], basePath);
