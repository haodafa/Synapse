...
export async function corruptSynapseConfig(repoPath: string): Promise<void> {
  await writeFile(path.join(repoPath, "synapse.json"), "{not valid json}");
}

export async function bumpSynapseConfigOnDisk(repoPath: string): Promise<void> {
  const configPath = path.join(repoPath, "synapse.json");
...
export async function restoreSynapseConfig(
  repoPath: string,
  config: Record<string, unknown>,
): Promise<void> {
  await writeFile(path.join(repoPath, "synapse.json"), JSON.stringify(config, null, 2) + "\n");
}
...
export async function blockSynapseConfigWrites(repoPath: string): Promise<void> {
  await chmod(repoPath, 0o555);
}

export async function unblockSynapseConfigWrites(repoPath: string): Promise<void> {
