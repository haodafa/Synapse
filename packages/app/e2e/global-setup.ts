...
import { forkSynapseHomeMetadata, resolveSynapseHomePath } from "./helpers/synapse-home-fork";
...
let synapseHome: string | null = null;
let fakeToolBinDir: string | null = null;
let relayProcess: ChildProcess | null = null;

function resolveOptionalSynapseHomeEnv(value: string | undefined): string | null {
  const trimmed = value?.trim();
  if (!trimmed) {
    return null;
  }
  if (trimmed === "current") {
    return resolveSynapseHomePath("~/.synapse");
  }
  return resolveSynapseHomePath(trimmed);
}
...
async function createFakeToolBin(): Promise<string> {
  const binDir = await mkdtemp(path.join(tmpdir(), "synapse-e2e-tool-bin-"));
  ...
  console.log(JSON.stringify([
    {
      ...
      url: "https://github.com/getsynapse/synapse/pull/515",
      state: "OPEN",
...
if (args[0] === "pr" && args[1] === "view" && args[2] === "--json" && args[3]) {
  const fixture = path.join(process.cwd(), ".synapse-e2e-pr.json");
...
if (args[0] === "api" && args[1] === "graphql") {
  const fixture = path.join(process.cwd(), ".synapse-e2e-timeline.json");
  if (fs.existsSync(fixture)) {
    console.log(fs.readFileSync(fixture, "utf8"));
    process.exit(0);
  }
...
const fs = require("fs");
const path = require("path");
const recordPath = process.env.SYNAPSE_E2E_EDITOR_RECORD_PATH;
...
function ensureRelayBuildArtifact(repoRoot: string): void {
  ...
  console.log("[e2e] Building @getsynapse/relay for daemon startup");
...
function loadPairingOfferFromCli(repoRoot: string, synapseHomePath: string): OfferPayload {
...
async function waitForPairingOfferFromCli(args: {
  ...
  while (Date.now() - start < timeoutMs) {
    try {
      return loadPairingOfferFromCli(args.repoRoot, args.synapseHome);
    } catch (error) {
      lastError = error;
      await sleep(100);
    }
  ...
  throw new Error(
    `Timed out waiting for \`synapse daemon pair --json\` to produce a pairing offer: ${
...
async function applySynapseHomeFork(targetHome: string): Promise<void> {
  const forkSourceHome = resolveOptionalSynapseHomeEnv(process.env.E2E_FORK_SYNAPSE_HOME_FROM);
  if (!forkSourceHome) {
    return;
  }
  const forkResult = await forkSynapseHomeMetadata({
    sourceHome: forkSourceHome,
    targetHome,
  });
  process.env.E2E_FORK_SOURCE_SYNAPSE_HOME = forkResult.sourceHome;
  process.env.E2E_FORK_TARGET_SYNAPSE_HOME = forkResult.targetHome;
  ...
  console.log(
    `[e2e] Forked Synapse metadata from ${forkResult.sourceHome} to ${forkResult.targetHome} ` +
  ...
  if (forkResult.skippedMissing.length > 0) {
    console.warn(
      `[e2e] Synapse metadata fork skipped missing paths: ${forkResult.skippedMissing.join(", ")}`,
    );
  }
}
...
async function resolveDictationConfig(): Promise<DictationConfig> {
  ...
  // them gate on SYNAPSE_DICTATION_ENABLED.
  if (!openAiUsable && !hasDefaultLocalModelsDir) {
    console.warn(
      "[e2e] Neither OPENAI_API_KEY nor local speech models found — running with dictation/voice disabled. " +
        "Tests that require dictation should gate on SYNAPSE_DICTATION_ENABLED.",
    );
...
function startDaemon(args: DaemonSpawnArgs): ChildProcess {
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      SYNAPSE_HOME: args.synapseHome,
      SYNAPSE_E2E_EDITOR_RECORD_PATH: args.editorRecordPath,
      SYNAPSE_SERVER_ID: "srv_e2e_test_daemon",
      SYNAPSE_LISTEN: `0.0.0.0:${args.port}`,
      SYNAPSE_RELAY_ENDPOINT: `127.0.0.1:${args.relayPort}`,
      SYNAPSE_CORS_ORIGINS: `http://localhost:${args.metroPort}`,
      SYNAPSE_DICTATION_ENABLED: openAiUsable ? "1" : "0",
      SYNAPSE_VOICE_MODE_ENABLED: openAiUsable ? "1" : "0",
      SYNAPSE_NODE_ENV: "development",
      ...(openAiUsable
        ? {
            SYNAPSE_DICTATION_STT_PROVIDER: "openai",
            SYNAPSE_VOICE_STT_PROVIDER: "openai",
            SYNAPSE_VOICE_TTS_PROVIDER: "openai",
          }
        : {}),
      ...(localModelsDir ? { SYNAPSE_LOCAL_MODELS_DIR: localModelsDir } : {}),
...
async function performCleanup(shouldRemoveSynapseHome: boolean): Promise<void> {
  await Promise.all([
  ...
  metroProcess = null;
  relayProcess = null;
  if (synapseHome && shouldRemoveSynapseHome) {
    await rm(synapseHome, { recursive: true, force: true });
    synapseHome = null;
  } else if (synapseHome) {
    console.log(`[e2e] Preserving SYNAPSE_HOME: ${synapseHome}`);
  }
  if (fakeToolBinDir) {
...
export default async function globalSetup() {
  ...
  const requestedSynapseHome = resolveOptionalSynapseHomeEnv(process.env.E2E_SYNAPSE_HOME);
  const shouldRemoveSynapseHome = !requestedSynapseHome && process.env.E2E_KEEP_SYNAPSE_HOME !== "1";
  synapseHome = requestedSynapseHome ?? (await mkdtemp(path.join(tmpdir(), "synapse-e2e-home-")));
  const editorRecordPath = path.join(synapseHome, "editor-open-records.jsonl");
  fakeToolBinDir = await createFakeToolBin();
  const metroLineBuffer = createLineBuffer();
  const daemonLineBuffer = createLineBuffer();

  await applySynapseHomeFork(synapseHome);

  const cleanup = () => performCleanup(shouldRemoveSynapseHome);
  ...
  try {
    ...
    process.env.E2E_SYNAPSE_HOME = synapseHome;
    process.env.E2E_EDITOR_RECORD_PATH = editorRecordPath;
    console.log(
      `[e2e] Test daemon started on port ${port}, Metro on port ${metroPort}, home: ${synapseHome}`,
    );
