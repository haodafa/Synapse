...
} from "@synapse/protocol/terminal-input-mode";
import {
...
export class TerminalEmulatorRuntime {
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    window.__synapseTerminal = terminal;
  ...
  unmount(): void {
    ...
    this.cleanup?.();
    this.cleanup = null;
    if (window.__synapseTerminal === this.terminal) {
      window.__synapseTerminal = undefined;
    }
    this.terminal = null;
    this.fitAddon = null;
