...
describe("desktop-updates helpers", () => {
  ...
  it("parses local daemon version error result", async () => {
    ...
    expect(
      parseLocalDaemonVersionResult({ version: null, error: "synapse command not found in PATH" }),
    ).toEqual({
      version: null,
      error: "synapse command not found in PATH",
    });
  });
  ...
  it("builds the direct Apple Silicon DMG URL from a version", async () => {
    ...
    expect(buildMacAppleSiliconDownloadUrl("v0.1.64")).toBe(
      "https://github.com/getsynapse/synapse/releases/download/v0.1.64/Synapse-0.1.64-arm64.dmg",
    );
