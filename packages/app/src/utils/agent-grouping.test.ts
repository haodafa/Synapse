...
describe("deriveProjectDisplayName", () => {
  it("shows owner/repo for GitHub remote keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "remote:github.com/getsynapse/synapse",
        projectName: "synapse",
      }),
    ).toBe("getsynapse/synapse");
  });
  ...
  it("falls back to projectName for local keys", () => {
    expect(
      deriveProjectDisplayName({
        projectKey: "/Users/me/dev/synapse",
        projectName: "synapse",
      }),
    ).toBe("synapse");
  });
...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/synapse" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/synapse-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:getsynapse/synapse.git",
    });
