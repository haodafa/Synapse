...
const SYNAPSE_WORKTREE_PATH_PATTERN = /(^|\/)\.synapse\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isSynapseOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isSynapseOwnedWorktreePath(cwd: string): boolean {
  return SYNAPSE_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
