...
const defaultCLIConfigPath = ".synapse/config.json"
...
// An empty profile returns the default path (~/.synapse/config.json).
// A named profile returns ~/.synapse/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".synapse", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.synapse/. A named profile returns ~/.synapse/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	return filepath.Join(home, ".synapse", "profiles", profile), nil
}
