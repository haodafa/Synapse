...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `synapse_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("synapse-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("synapse_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/synapse-ai/synapse/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the synapse GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/synapse-ai/synapse/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running synapse binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade synapse-ai/tap/synapse`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "synapse")
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	tmpFile, err := os.CreateTemp(dir, "synapse-update-*")
	if err != nil {
