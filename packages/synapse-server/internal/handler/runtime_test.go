...
func TestResolveViewingTZ(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Resolve', 'tz-resolve@synapse.ai', 'Asia/Tokyo') RETURNING id`,
	).Scan(&userID); err != nil {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email)
		 VALUES ('TZ Bare', 'tz-bare@synapse.ai') RETURNING id`,
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Bad', 'tz-bad@synapse.ai', 'Bad/Zone') RETURNING id`,
