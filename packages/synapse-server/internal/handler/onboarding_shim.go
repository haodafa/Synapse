...
import (
	...
	"github.com/haodafa/Synapse/server/internal/issueguard"
...
const (
	onboardingAssistantName = "Synapse Helper"
	onboardingIssueTitle    = "Start here: learn Synapse with Synapse Helper"
	onboardingAgentTemplate = "synapse_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Synapse questions and runs CLI operations."
...
// Synapse Helper agent created by this shim. Pre-v3 desktop submits a
...
// is removed.
const onboardingAssistantInstructions = `You are Synapse Helper, the built-in AI assistant for this Synapse workspace. Your role is to help any member use Synapse better — answer questions, give advice, and execute workspace operations on their behalf.

## What Synapse is

Synapse is an open-source, AI-native team workspace (source: https://github.com/haodafa/Synapse). The core idea: AI agents are treated as real teammates — they get assigned issues on a kanban-style board, comment in threads, change status, and run code, exactly like human members. You can also chat directly with agents (chat), group them into squads, and run scheduled or triggered automation (autopilot).
...
For ANY product-usage problem the user runs into (bug, unclear behavior, missing feature, improvement idea), suggest they file an issue at https://github.com/haodafa/Synapse/issues — that's the official feedback channel.
...
const onboardingIssueDescription = `Welcome to Synapse.

This is your guided first run. Synapse Helper is assigned to this issue and will help you try the core workflow:

1. Read Synapse Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Synapse Helper when you want it to continue.
...
//
// Creates or reuses one "Synapse Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Synapse.",
		"",
		"Agents need a runtime before they can execute work. You can still use Synapse as a lightweight project-management workspace while you install one.",
		"",
		"## Try Synapse first",
		"",
		...
		"",
		"Full guide: https://synapse.ai/docs/install-agent-runtime",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Synapse daemon:",
		"   synapse daemon restart",
		...
		"",
		"When the runtime is connected, you can create Synapse Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Synapse。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Synapse 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://synapse.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Synapse Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
