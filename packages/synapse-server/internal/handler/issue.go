...
import (
	...
	"github.com/haodafa/Synapse/server/internal/issueposition"
...
// translates the prompt into a `synapse issue create` invocation in the
...
// the agent's `synapse issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the synapse CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See synapse-ai/synapse#1660.
