...
import (
	...
	"github.com/haodafa/Synapse/server/internal/mention"
...
// `synapse issue create` call. Pre-validates that the agent is reachable
...
// manual `synapse agent update <id> --status idle` to unwedge. Matches the
...
// daemon-injected SYNAPSE_QUICK_CREATE_TASK_ID env var, so this lookup is
