...
// same machine shares one UUID stored at `~/.synapse/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.synapse/profiles/*/daemon.id` and returns every
...
// explicitly pins SYNAPSE_DAEMON_ID to the hostname itself, there's nothing
