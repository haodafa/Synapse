...
const sidecarManifestFile = ".synapse_sidecar_manifest.json"
...
//     so the agent still discovers the Synapse skill, just under a
//     different directory name.
//   - For Synapse-only namespaces (.agent_context/issue_context.md,
//     .synapse/project/resources.json) the caller swallows the error
...
// down a Synapse skill without colliding with a user-installed skill
...
// collision we append `-synapse`, then `-synapse-2`, `-synapse-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + ".synapse"
		default:
			candidate = fmt.Sprintf("%s-synapse-%d", baseSlug, i)
		}
...
// (.agent_context/, .synapse/, .claude/skills/, .github/skills/,
