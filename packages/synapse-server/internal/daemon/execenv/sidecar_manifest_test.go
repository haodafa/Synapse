...
// must still exist and the Synapse-written sibling must be gone.
...
// provider, seed a user skill at the exact slug Synapse would use
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Synapse's version",
						Content:     "---\nname: issue-review\n---\n\nSynapse skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Synapse-only namespace file. If the user already has a file at
...
// `Issue Review` skill, the Synapse sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Synapse sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Synapse skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Synapse's version\n"},
		},
	}, manifest); err != nil {
	...
	// Synapse sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
