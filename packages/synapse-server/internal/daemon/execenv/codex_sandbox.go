...
// `no such host` errors when calling out (for example, `synapse issue get`
// hitting the Synapse API). See upstream issue openai/codex#10390.
//
...
// actually reach the Synapse API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Synapse CLI can reach the API.
...
// synapseManagedBeginMarker / synapseManagedEndMarker delimit the block the
...
const (
	synapseManagedBeginMarker = "# BEGIN synapse-managed (do not edit; regenerated by daemon)"
	synapseManagedEndMarker   = "# END synapse-managed"
)

// renderSynapseManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.synapse]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertSynapseManagedBlock), avoids both traps.
func renderSynapseManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(synapseManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(synapseManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(synapseManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(synapseManagedEndMarker) + `\n*`)

// upsertSynapseManagedBlock returns the config content with the synapse-managed
...
// `[permissions.synapse]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderSynapseManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertSynapseManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderSynapseManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the synapse-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertSynapseManagedBlock(existing, policy)
