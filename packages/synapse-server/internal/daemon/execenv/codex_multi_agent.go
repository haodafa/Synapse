...
// subagents. The Synapse daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Synapse runtime
...
//
// Users who explicitly want Codex native subagents inside a Synapse task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `SYNAPSE_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// SynapseCodexMultiAgentEnv is the env var users can set to keep Codex
...
const SynapseCodexMultiAgentEnv = "SYNAPSE_CODEX_MULTI_AGENT"

// synapseMultiAgentBeginMarker / synapseMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	synapseMultiAgentBeginMarker = "# BEGIN synapse-managed multi-agent (do not edit; regenerated by daemon)"
	synapseMultiAgentEndMarker   = "# END synapse-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(synapseMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(synapseMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(SynapseCodexMultiAgentEnv))
...
// renderSynapseMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderSynapseMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(synapseMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(synapseMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderSynapseMultiAgentBlock(true)
...
//
// When SYNAPSE_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per SYNAPSE_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderSynapseMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
