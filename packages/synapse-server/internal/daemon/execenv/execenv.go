...
// checked out on demand by the agent via `synapse repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string // base path for all envs (e.g., ~/synapse_workspaces)
...
// on demand via `synapse repo checkout <url>`.
