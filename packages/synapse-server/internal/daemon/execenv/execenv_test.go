...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Synapse Agent Runtime") {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	// inherit from any surrounding table scope. See upsertSynapseManagedBlock
	// for why.
...
// inside a Synapse task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
