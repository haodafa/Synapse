...
// — users cannot audit or edit the contents from any Synapse UI.
//
// This conflicts with the Synapse daemon's context model. Synapse
...
//     unrelated host-project context into Synapse tasks. The reproduction
//     in github.com/haodafa/Synapse#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Synapse issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Synapse task
// (and accept the leak risk) can keep the feature enabled by setting
// `SYNAPSE_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Synapse-owned, user-visible,
...
// SynapseCodexMemoryEnv is the env var users can set to keep Codex
...
const SynapseCodexMemoryEnv = "SYNAPSE_CODEX_MEMORY"

const (
	synapseMemoryFeatureBeginMarker = "# BEGIN synapse-managed memory-feature (do not edit; regenerated by daemon)"
	synapseMemoryFeatureEndMarker   = "# END synapse-managed memory-feature"
	synapseMemoryConfigBeginMarker  = "# BEGIN synapse-managed memory-config (do not edit; regenerated by daemon)"
	synapseMemoryConfigEndMarker    = "# END synapse-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(synapseMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(synapseMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(synapseMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(synapseMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(SynapseCodexMemoryEnv))
...
// renderSynapseMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderSynapseMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(synapseMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(synapseMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderSynapseMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderSynapseMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(synapseMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(synapseMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderSynapseMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderSynapseMemoryConfigBlock(true))
}
...
//
// When SYNAPSE_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per SYNAPSE_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderSynapseMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderSynapseMemoryConfigBlock(false))
	}
