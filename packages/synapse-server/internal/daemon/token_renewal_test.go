...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "synapse login") {
		t.Fatalf("401 warning must tell the user to run 'synapse login', got: %s", out)
	}
}
...
// and the operator would never see a "run synapse login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "synapse login") {
		t.Fatalf("expected the actionable 'run synapse login' hint in the WARN, got: %s", out)
	}
}
