...
const (
	...
	// Set SYNAPSE_AGENT_IDLE_WATCHDOG=0 to disable.
...
// SYNAPSE_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // synapse CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/synapse_workspaces)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on Synapse Cloud, false on self-host)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("SYNAPSE_SERVER_URL", DefaultServerURL)
	...
	// pinned to an explicit SYNAPSE_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned SYNAPSE_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("SYNAPSE_CLAUDE_PATH", "claude", "SYNAPSE_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("SYNAPSE_CODEX_PATH", "codex", "SYNAPSE_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("SYNAPSE_OPENCODE_PATH", "opencode", "SYNAPSE_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("SYNAPSE_OPENCLAW_PATH", "openclaw", "SYNAPSE_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("SYNAPSE_HERMES_PATH", "hermes", "SYNAPSE_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("SYNAPSE_GEMINI_PATH", "gemini", "SYNAPSE_GEMINI_MODEL"); ok {
		agents["gemini"] = e
	}
	if e, ok := probe("SYNAPSE_PI_PATH", "pi", "SYNAPSE_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("SYNAPSE_CURSOR_PATH", "cursor-agent", "SYNAPSE_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("SYNAPSE_COPILOT_PATH", "copilot", "SYNAPSE_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("SYNAPSE_KIMI_PATH", "kimi", "SYNAPSE_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("SYNAPSE_KIRO_PATH", "kiro-cli", "SYNAPSE_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	...
	if e, ok := probe("SYNAPSE_ANTIGRAVITY_PATH", "agy", ""); ok {
	...
	claudeArgs, err := shellArgsFromEnv("SYNAPSE_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("SYNAPSE_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("SYNAPSE_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("SYNAPSE_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("SYNAPSE_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("SYNAPSE_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// SYNAPSE_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("SYNAPSE_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("SYNAPSE_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via SYNAPSE_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("SYNAPSE_DAEMON_ID"))
	...
	// — e.g. ~/.synapse/daemon.id (default) plus ~/.synapse/profiles/<x>/
	...
	// the user explicitly pins SYNAPSE_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("SYNAPSE_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("SYNAPSE_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/synapse_workspaces or ~/synapse_workspaces_<profile>)
	...
	keepEnv := os.Getenv("SYNAPSE_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("SYNAPSE_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("SYNAPSE_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("SYNAPSE_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("SYNAPSE_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("SYNAPSE_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("SYNAPSE_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("SYNAPSE_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on Synapse Cloud (api.synapse.ai) and opt-out for
	...
	// Operators on either side can flip the default with SYNAPSE_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("SYNAPSE_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("SYNAPSE_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of Synapse's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.synapse.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.synapse.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at Synapse's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid SYNAPSE_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("SYNAPSE_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// SYNAPSE_WORKSPACES_ROOT env > default ($HOME/synapse_workspaces, or
// $HOME/synapse_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `synapse daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("SYNAPSE_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set SYNAPSE_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "synapse_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "synapse_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("SYNAPSE_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// before any SYNAPSE_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom SYNAPSE_*_PATH values never reach this
