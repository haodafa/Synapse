...
import (
	...
	"github.com/haodafa/Synapse/server/internal/daemon/repocache"
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'synapse login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'synapse login --profile %s'", d.cfg.Profile)
		}
...
// `synapse repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run synapse login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Synapse Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/synapse)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/synapse symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "synapse")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "synapse")
		} else {
...
// release. Used by pollLoop to expose SYNAPSE_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// here would make SYNAPSE_WORKSPACE_ID empty in the agent env, and the
	...
	// `synapse repo checkout` would reject project-only URLs that aren't also
	...
	// up stale Synapse instructions (issue id, trigger comment id, reply
	...
	if env.LocalDirectory {
		defer func() {
			...
			// Excise the sidecar tree (.agent_context/, .synapse/,
	...
	// can call the Synapse API and the local daemon (e.g. `synapse repo checkout`).
	// SYNAPSE_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// SYNAPSE_TOKEN is the credential the agent process will use to call the
	// Synapse API. Prefer the task-scoped token the server minted at claim
	...
	agentEnv := map[string]string{
		"SYNAPSE_TOKEN":        agentToken,
		"SYNAPSE_SERVER_URL":   d.cfg.ServerBaseURL,
		"SYNAPSE_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"SYNAPSE_WORKSPACE_ID": task.WorkspaceID,
		"SYNAPSE_AGENT_NAME":   agentName,
		"SYNAPSE_AGENT_ID":     task.AgentID,
		"SYNAPSE_TASK_ID":      task.ID,
		"SYNAPSE_TASK_SLOT":    strconv.Itoa(slot),
	}
	if task.AutopilotRunID != "" {
		agentEnv["SYNAPSE_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["SYNAPSE_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the synapse CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["SYNAPSE_QUICK_CREATE_TASK_ID"] = task.ID
	}
	// Ensure the synapse CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// synapse binary so that `synapse` commands in the agent always resolve.
	...
	// then the daemon-wide SYNAPSE_<PROVIDER>_MODEL env var. If
	...
	// `synapse issue status` / `synapse issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "SYNAPSE_") {
		return true
	}
	switch upper {
