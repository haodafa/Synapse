...
// synapseHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
// treats a hook as Synapse-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before synapseHookMarker existed, so disabling the toggle on
...
// Co-authored-by trailer for the Synapse Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# synapse:prepare-commit-msg:co-authored-by
...
// a Co-authored-by trailer for the Synapse Agent. The hook is installed in the
