...
// for the Synapse Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: synapse-agent <github@synapse.ai>"
...
func TestCoAuthoredByHookIdempotent(t *testing.T) {
	...
	trailer := "Co-authored-by: synapse-agent <github@synapse.ai>"
	if err := os.WriteFile(filepath.Join(result.Path, "test.txt"), []byte("hello\n"), 0o644); err != nil {
		t.Fatalf("write test file: %v", err)
	}
	runGitAuthored(t, result.Path, "add", ".")
...
// path: a bare cache that already carries the Synapse prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: synapse-agent") {
...
# Synapse: add Co-authored-by trailer for the Synapse Agent.
# Installed by the Synapse daemon. Do not edit — it will be overwritten.
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: synapse-agent") {
...
// without the Synapse marker (e.g. one a user added manually) must be left
