...
func NewRealtimeCollector(m *realtime.Metrics) *RealtimeCollector {
	return &RealtimeCollector{
		...
		redisMirrorErrors:   prometheus.NewDesc("synapse_realtime_redis_mirror_errors_total", "Total Redis mirror write errors by the realtime relay.", []string{"target"}, nil),
...
func newRealtimeDesc(name, help string) *prometheus.Desc {
	return prometheus.NewDesc("synapse_realtime_"+name, help, nil, nil)
}
