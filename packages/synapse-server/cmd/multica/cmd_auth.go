...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate synapse with Synapse",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("SYNAPSE_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'synapse setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'synapse login' to authenticate.")
		return nil
	}
...
<title>Synapse — Authenticated</title>
<style>
