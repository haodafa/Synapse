...
var rootHelpTemplate = `Work seamlessly with Synapse from the command line.

USAGE
  synapse <command> <subcommand> [flags]
{{range .Groups}}
{{.Title}}
...
EXAMPLES
  $ synapse login
  $ synapse issue list --output json
  $ synapse daemon start
  $ synapse agent list --output json

ENVIRONMENT VARIABLES
  SYNAPSE_SERVER_URL    Override the default server URL
  SYNAPSE_WORKSPACE_ID  Set the active workspace

LEARN MORE
  Use ` + "`synapse <command> <subcommand> --help`" + ` for more information about a command.
`
