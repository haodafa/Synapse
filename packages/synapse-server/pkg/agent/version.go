...
// the synapse CLI version reported by the daemon at registration time. The
...
// `synapse issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("synapse CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("synapse CLI version is below required minimum")
)
