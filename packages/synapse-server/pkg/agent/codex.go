...
const (
	synapseCodexMcpBeginMarker = "# BEGIN synapse-managed mcp_servers (do not edit; regenerated by daemon)"
	synapseCodexMcpEndMarker   = "# END synapse-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(synapseCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(synapseCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = synapseCodexMcpBeginMarker + "\n" + synapseCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(synapseCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(synapseCodexMcpEndMarker)
	sb.WriteString("\n")
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "synapse-agent-sdk",
				"title":   "Synapse Agent SDK",
				"version": "0.2.0",
			},
