...
vi.mock("@synapse/ui/components/ui/sidebar", () => ({
...
vi.mock("@synapse/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@synapse/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));

vi.mock("@synapse/core/auth", () => ({
  useAuthStore: (selector: (state: { user: { id: string } }) => unknown) => selector({ user: { id: "user-1" } }),
}));
vi.mock("@synapse/core/paths", () => ({
...
vi.mock("@synapse/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@synapse/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@synapse/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@synapse/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@synapse/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@synapse/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@synapse/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@synapse/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@synapse/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@synapse/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@synapse/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
vi.mock("@synapse/core/workspace/queries", () => ({
