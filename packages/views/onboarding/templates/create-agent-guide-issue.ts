...
const HELPER_AGENT_NAME = "Synapse Helper";

/**
 * Skip path, issue 2/2: "Create your first Synapse Agent".
 *
...
export const CREATE_AGENT_GUIDE_ISSUE_TITLE = {
  en: "Step 2 — Create your first Synapse Agent",
  zh: "第 2 步 —— 创建你的第一个 Synapse Agent",
} as const;
...
function enBody(installRuntimeMention: string): string {
  return `Once your runtime is online (see ${installRuntimeMention}), build your first agent — Synapse Helper. The prompt below is pre-written; just copy.
...
Now create an issue (or reassign an existing one) → set assignee = Synapse Helper → set status to **todo**. The runtime picks the task up within a few seconds and starts working. Watch progress in the issue's task panel.
...
- **Docs** — https://synapse.ai/docs.`;
}

function zhBody(installRuntimeMention: string): string {
  return `等运行时上线（见 ${installRuntimeMention}）之后，把第一个 agent —— Synapse Helper —— 建出来。下面的提示词已经写好，直接复制即可。
...
接着创建一个 issue（或把已有 issue 重新分派）→ 把 assignee 设成 Synapse Helper → 状态切到 **todo**。运行时会在几秒内接走任务并开始工作。在 issue 的任务面板里看进度。
