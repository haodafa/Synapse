...
import type { Issue, TimelineEntry } from "@synapse/core/types";
...
vi.mock("@synapse/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @synapse/core/hooks.tsx). vi.mock("@synapse/core/paths") only intercepts
...
// Mock @synapse/core/workspace/hooks
vi.mock("@synapse/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @synapse/core/paths — after the URL-driven workspace refactor,
...
vi.mock("../../navigation", () => ({
  ...
  useNavigation: () => ({
    ...
    getShareableUrl: (p: string) => `https://app.synapse.com${p}`,
  }),
...
// Mock issue config
vi.mock("@synapse/core/issues/config", () => ({
...
vi.mock("@synapse/core/issues/stores", () => ({
...
// Mock react-resizable-panels (used by @synapse/ui/components/ui/resizable)
