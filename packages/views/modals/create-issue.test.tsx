...
vi.mock("@synapse/core/hooks", () => ({
  useWorkspaceId: () => "ws-test",
}));

vi.mock("@synapse/core/issues/queries", () => ({
...
vi.mock("@synapse/core/issues/stores/draft-store", () => ({
...
vi.mock("@synapse/core/issues/stores/quick-create-store", () => ({
  useQuickCreateStore: (selector?: (state: typeof mockQuickCreateStore) => unknown) =>
    (selector ? selector(mockQuickCreateStore) : mockQuickCreateStore),
}));

vi.mock("@synapse/core/issues/mutations", () => ({
  useCreateIssue: () => ({ mutateAsync: mockCreateIssue }),
  useUpdateIssue: () => ({ mutate: vi.fn() }),
}));

vi.mock("@synapse/core/hooks/use-file-upload", () => ({
...
vi.mock("@synapse/core/api", async () => {
  ...
  const { parseWithFallback } = await vi.importActual<typeof import("@synapse/core/api/schema")>(
    "@synapse/core/api/schema",
  );
  const { DuplicateIssueErrorBodySchema } = await vi.importActual<
    typeof import("@synapse/core/api/schemas")
  >("@synapse/core/api/schemas");
  return {
    api: {},
    ApiError,
    parseWithFallback,
...
vi.mock("@synapse/ui/components/ui/dialog", () => ({
...
vi.mock("@synapse/ui/components/ui/button", () => ({
  Button: ({
    children,
    disabled,
    onClick,
...
vi.mock("@synapse/ui/components/ui/switch", () => ({
  Switch: ({
    checked,
    onCheckedChange,
  }: {
...
vi.mock("@synapse/ui/components/common/file-upload-button", () => ({
