...
describe("LoginPage", () => {
  ...
  it("renders email form with 'Sign in to Synapse' title", () => {
    renderWithI18n(<LoginPage onSuccess={onSuccess} />);
    expect(
      screen.getByText(/sign in to synapse/i),
    ).toBeInTheDocument();
    expect(
  ...
  it("shows cli_confirm step when existing session + cliCallback", async () => {
    localStorage.setItem("synapse_token", "existing-jwt");
