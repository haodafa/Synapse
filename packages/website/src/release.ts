...
const REQUIRED_ASSET_PATTERNS = [
  /Synapse-.*-arm64\.dmg$/, // Mac Apple Silicon
  /Synapse-.*-x86_64\.AppImage$/, // Linux AppImage
  /Synapse-Setup-.*\.exe$/, // Windows (any arch)
];
...
function pickWindowsAssets(assets: GitHubAsset[]) {
  const x64Suffixed = assets.find((a) => /Synapse-Setup-.*-x64\.exe$/.test(a.name));
  const arm64 = assets.find((a) => /Synapse-Setup-.*-arm64\.exe$/.test(a.name));
  const legacy = assets.find(
    (a) =>
      /Synapse-Setup-.*\.exe$/.test(a.name) &&
...
const GITHUB_RELEASES_URL = "https://api.github.com/repos/getsynapse/synapse/releases?per_page=10";
...
function isReleaseInfo(value: unknown): value is ReleaseInfo {
  ...
  return (
    ...
    (record.windowsX64Asset === null ||
      new RegExp(`^Synapse-Setup-${record.version.replaceAll(".", "\\.")}(?:-x64)?\\.exe$`).test(
        record.windowsX64Asset,
      )) &&
    (record.windowsArm64Asset === null ||
      new RegExp(`^Synapse-Setup-${record.version.replaceAll(".", "\\.")}-arm64\\.exe$`).test(
