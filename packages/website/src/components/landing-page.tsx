...
function MultiProviderSection() {
  ...
  return (
    <FeatureSection
      ...
      description="Run multiple providers from a single interface. Synapse runs the native agent harness as you'd normally run it, with your skills, config and MCP servers intact."
    >
...
function ServiceProxySection() {
  ...
  return (
    <FeatureSection
      ...
      description="When agents work in parallel, they all run dev servers. Synapse gives each one a URL based on the branch name, no port conflicts, no guessing."
    >
...
const SERVER_INSTALL_FOOTNOTE = (
  <>
    Requires Node.js 18+. Run <span className="font-mono text-white/40">synapse</span> to start the
    daemon.
  </>
);
...
const bashCommands = new Set(["synapse", "echo", "jq"]);
...
const cliExamples: CLIExample[] = [
  {
    ...
    code: `synapse run "implement user authentication"
synapse run --provider codex --worktree feature-x "implement feature X"
synapse run --host devbox:6767 "run the full test suite"

synapse ls                           # list running agents
synapse attach abc123                # stream live output
synapse send abc123 "also add tests" # follow-up task`,
  },
  {
    title: "Loops",
    description:
...
synapse loop run "make all tests pass" \\
  --verify "verify tests pass and the code is production-ready" \\
  --verify-check "npm test" \\
  --max-iterations 5

synapse loop ls                        # list running loops
synapse loop logs abc123               # stream loop output`,
  },
  {
    title: "Schedules",
    description:
...
synapse schedule create --cron "0 9 * * 1" \\
  "audit the codebase for security issues and open PRs for fixes"

synapse schedule ls                    # list all schedules
synapse schedule pause abc123          # pause a schedule
synapse schedule delete abc123         # remove a schedule`,
  },
];
    ...
    >
      ...
      <div className="space-y-6">
        <FAQItem question="Is this free?">
          Yes. Synapse is free and open source. You need Claude Code, Codex, Copilot, OpenCode, or Pi
        ...
        <FAQItem question="Does my code leave my machine?">
          Synapse doesn&apos;t send your code anywhere. Agents run locally and talk to their own APIs
        ...
        <FAQItem question="What agents does it support?">
          ...
          its own CLI or local integration. Synapse doesn&apos;t modify or wrap their behavior.
        </FAQItem>
        ...
        <FAQItem question="Can I connect from outside my network?">
          Yes. You can use the hosted relay (end-to-end encrypted, Synapse can&apos;t read your
        ...
        <FAQItem question="Do I need git or GitHub?">
          No. Synapse works in any directory. Worktrees are optional and only relevant if you use git.
          You can run agents anywhere you&apos;d normally work.
        </FAQItem>
        <FAQItem question="Can I get banned for using Synapse?">
          <p>We can&apos;t make promises on behalf of providers.</p>
          <p>
            That said, Synapse launches each provider&apos;s local CLI or integration (Claude Code,
            Codex, Copilot, OpenCode, Pi) as a subprocess. It doesn&apos;t extract tokens or call
            inference APIs directly. From the provider&apos;s perspective, usage through Synapse is
            indistinguishable from running the provider yourself.
          </p>
          <p>I&apos;ve been using Synapse with all providers for months without issue.</p>
        </FAQItem>
        <FAQItem question="How do worktrees work?">
          When you launch an agent with the worktree option (from the app, desktop, or CLI), Synapse
  ...
  return (
    ...
    >
      <div className="text-sm text-muted-foreground leading-relaxed space-y-3">
        <p>
          I built Synapse because I wanted better tools for coding agents on my own setup. It&apos;s
