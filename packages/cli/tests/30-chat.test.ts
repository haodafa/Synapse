...
try {
...

  {
    ...
    const posted = await ctx.synapse(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { SYNAPSE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.synapse(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.synapse(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.synapse(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.synapse(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.synapse(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.synapse(["chat", "delete", "coord-room"]);
  ...
  await ctx.stop();
  await rm(ctx.synapseHome, { recursive: true, force: true });
