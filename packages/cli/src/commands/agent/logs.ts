...
import type { AgentStreamMessage } from "@synapse/protocol/messages";
import { curateAgentActivity } from "@synapse/unified-daemon/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: synapse agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: synapse daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `synapse ls` to list available agents");
      await client.close();
      process.exit(1);
    }
